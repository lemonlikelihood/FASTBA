...
Matrix3d find_essential_matrix(
    const std::vector<Vector2d> &points1, const std::vector<Vector2d> &points2, double threshold,
    double confidence, size_t max_iteration, int seed) {
    std::vector<char> _;
    return find_essential_matrix(points1, points2, _, threshold, confidence, max_iteration, seed);
}

Matrix3d find_homography_matrix(
    const std::vector<Vector2d> &points1, const std::vector<Vector2d> &points2, double threshold,
    double confidence, size_t max_iteration, int seed) {
    std::vector<char> _;
...
Matrix3d find_essential_matrix(
    ...
    std::vector<char> &inlier_mask, double threshold, double confidence, size_t max_iteration,
    int seed) {
    struct EssentialSolver {
        std::vector<Matrix3d> operator()(
            const std::array<Vector2d, 5> &samples1,
            const std::array<Vector2d, 5> &samples2) const {
    ...
    struct EssentialEvaluator {
        const Matrix3d &E;
        const Matrix3d Et;
        EssentialEvaluator(const Matrix3d &E) : E(E), Et(E.transpose()) {}
    ...
    Ransac<5, Matrix3d, EssentialSolver, EssentialEvaluator> ransac(
...
Matrix3d find_homography_matrix(
    ...
    struct HomographySolver {
        Matrix3d operator()(
            const std::array<Vector2d, 4> &samples1,
            const std::array<Vector2d, 4> &samples2) const {
    ...
    struct HomographyEvaluator {
        const Matrix3d &H;
        const Matrix3d Hinv;
        HomographyEvaluator(const Matrix3d &H) : H(H), Hinv(H.inverse()) {}
        double operator()(const Vector2d &p1, const Vector2d &p2) const {
            return homography_geometric_error(H, p1, p2) + homography_geometric_error(Hinv, p2, p1);
        }
    };
    static const double t2 = 5.99;
    Ransac<4, Matrix3d, HomographySolver, HomographyEvaluator> ransac(
        2.0 * t2 * threshold * threshold, confidence, max_iteration, seed);
    return ransac.solve(inlier_mask, points1, points2);
}

size_t triangulate_from_rt(
    const std::vector<Vector2d> &points1, const std::vector<Vector2d> &points2, const Matrix3d &R,
    const Vector3d &T, std::vector<Vector3d> &result_points, std::vector<char> &result_status) {
    size_t count = 0;
...
size_t triangulate_from_rt(
    const std::vector<Vector2d> &points1, const std::vector<Vector2d> &points2,
    const std::vector<Matrix3d> &Rs, const std::vector<Vector3d> &Ts,
...
size_t triangulate_from_rt_scored(
    const std::vector<Vector2d> &points1, const std::vector<Vector2d> &points2, const Matrix3d &R,
    const Vector3d &T, std::vector<Vector3d> &result_points, std::vector<char> &result_status,
    double &score) {
    size_t count = 0;
    ...
    for (size_t i = 0; i < points1.size(); ++i) {
        ...
        if (triangulate_point_scored(
                P1, P2, points1[i], points2[i], result_points[i], current_score)) {
            result_status[i] = 1;
...
size_t triangulate_from_rt_scored(
    ...
    size_t best_i = 0;
    for (size_t i = 0; i < Rs.size(); ++i) {
        counts[i] = triangulate_from_rt_scored(
            points1, points2, Rs[i], Ts[i], points[i], status[i], scores[i]);
...
size_t triangulate_from_essential(
    const std::vector<Vector2d> &points1, const std::vector<Vector2d> &points2, const Matrix3d &E,
    std::vector<Vector3d> &result_points, std::vector<char> &result_status, Matrix3d &R,
    Vector3d &T) {
...
size_t triangulate_from_homography(
    const std::vector<Vector2d> &points1, const std::vector<Vector2d> &points2, const Matrix3d &H,
    std::vector<Vector3d> &result_points, std::vector<char> &result_status, Matrix3d &R,
    Vector3d &T) {
    std::array<Matrix3d, 4> Rs;
    std::array<Vector3d, 4> Ts;
