...
Eigen::Matrix3d find_essential_matrix(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    double threshold = 1.0, double confidence = 0.999, size_t max_iteration = 1000, int seed = 0);
Eigen::Matrix3d find_homography_matrix(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    double threshold = 1.0, double confidence = 0.999, size_t max_iteration = 1000, int seed = 0);
Eigen::Matrix3d find_essential_matrix(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    std::vector<char> &inlier_mask, double threshold = 1.0, double confidence = 0.999,
...
Eigen::Vector4d triangulate_point(
    const Eigen::Matrix<double, 3, 4> &P1, const Eigen::Matrix<double, 3, 4> &P2,
    const Eigen::Vector2d &point1, const Eigen::Vector2d &point2);
Eigen::Vector4d triangulate_point(
    const std::vector<Eigen::Matrix<double, 3, 4>> &Ps, const std::vector<Eigen::Vector2d> &points);

bool triangulate_point_checked(
    const Eigen::Matrix<double, 3, 4> &P1, const Eigen::Matrix<double, 3, 4> &P2,
    const Eigen::Vector2d &point1, const Eigen::Vector2d &point2, Eigen::Vector3d &p);
bool triangulate_point_checked(
    const std::vector<Eigen::Matrix<double, 3, 4>> &Ps, const std::vector<Eigen::Vector2d> &points,
    Eigen::Vector3d &p);
bool triangulate_point_scored(
    const Eigen::Matrix<double, 3, 4> &P1, const Eigen::Matrix<double, 3, 4> &P2,
    const Eigen::Vector2d &point1, const Eigen::Vector2d &point2, Eigen::Vector3d &p,
    double &score);
bool triangulate_point_scored(
    const std::vector<Eigen::Matrix<double, 3, 4>> &Ps, const std::vector<Eigen::Vector2d> &points,
    Eigen::Vector3d &p, double &score);

size_t triangulate_from_rt(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    const Eigen::Matrix3d &R, const Eigen::Vector3d &T, std::vector<Eigen::Vector3d> &result_points,
    std::vector<char> &result_status);
size_t triangulate_from_rt(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    const std::vector<Eigen::Matrix3d> &Rs, const std::vector<Eigen::Vector3d> &Ts,
    std::vector<Eigen::Vector3d> &result_points, Eigen::Matrix3d &result_R,
    Eigen::Vector3d &result_T, std::vector<char> &result_status);

size_t triangulate_from_rt_scored(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    const Eigen::Matrix3d &R, const Eigen::Vector3d &T, std::vector<Eigen::Vector3d> &result_points,
    std::vector<char> &result_status, double &score);
size_t triangulate_from_rt_scored(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    const std::vector<Eigen::Matrix3d> &Rs, const std::vector<Eigen::Vector3d> &Ts,
...
size_t triangulate_from_essential(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    const Eigen::Matrix3d &E, std::vector<Eigen::Vector3d> &result_points,
    std::vector<char> &result_status, Eigen::Matrix3d &R, Eigen::Vector3d &T);
size_t triangulate_from_homography(
    const std::vector<Eigen::Vector2d> &points1, const std::vector<Eigen::Vector2d> &points2,
    const Eigen::Matrix3d &H, std::vector<Eigen::Vector3d> &result_points,
    std::vector<char> &result_status, Eigen::Matrix3d &R, Eigen::Vector3d &T);

// 两帧图像三角化，利用叉乘原理，返回三维点的齐次坐标
inline Eigen::Vector4d triangulate_point(
    const Eigen::Matrix<double, 3, 4> &P1, const Eigen::Matrix<double, 3, 4> &P2,
    const Eigen::Vector2d &point1, const Eigen::Vector2d &point2) {
    Eigen::Matrix4d A;
...
inline bool triangulate_point_checked(
    const Eigen::Matrix<double, 3, 4> &P1, const Eigen::Matrix<double, 3, 4> &P2,
    const Eigen::Vector2d &point1, const Eigen::Vector2d &point2, Eigen::Vector3d &p) {
    double score;
...
inline bool triangulate_point_scored(
    const Eigen::Matrix<double, 3, 4> &P1, const Eigen::Matrix<double, 3, 4> &P2,
    const Eigen::Vector2d &point1, const Eigen::Vector2d &point2, Eigen::Vector3d &p,
    double &score) {
    Eigen::Vector4d q = triangulate_point(
        P1, P2, point1, point2); // 两帧图像三角化，利用叉乘原理，返回三维点的齐次坐标
    score = 0;
    ...
    if (q1[2] * q[3] > 0 && q2[2] * q[3] > 0) { // 在同一侧
        if (q1[2] / q[3] < 100 && q2[2] / q[3] < 100) {
            ...
            score = 0.5
                    * ((q1.hnormalized() - point1).squaredNorm()
                       + (q2.hnormalized() - point2).squaredNorm());
...
inline bool triangulate_point_scored(
    ...
    if (Ps.size() < 2)
        return false;
    Eigen::Vector4d q = triangulate_point(
        Ps, points); // 每一帧观测数据都会提供两个约束方程，至少需要两个观测才能三角化
    score = 0;
