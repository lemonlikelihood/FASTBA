...
public:
    ...
    // [left, right]
    UniformInteger(
        value_type left = value_type(0), value_type right = std::numeric_limits<T>::max())
        : distribution(left, right) {}
    ...
    value_type next(value_type left, value_type right) {
        return distribution(
            engine, typename std::uniform_int_distribution<value_type>::param_type(left, right));
    }
...
class LotBox {
public:
    LotBox(size_t size) : cap(0), lots(size) { std::iota(lots.begin(), lots.end(), 0); }
