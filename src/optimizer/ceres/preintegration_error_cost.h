...
class PreIntegrationErrorCost : public Factor::FactorCostFunction,
...
public:
    ...
    bool Evaluate(
        ...
        r.segment<3>(ES_Q) =
            logmap((dq * expmap(dq_dbg * dbg)).conjugate() * q_i.conjugate() * q_j);
        r.segment<3>(ES_P) = q_i.conjugate() * (p_j - p_i - dt * v_i - 0.5 * dt * dt * gravity)
                             - (dp + dp_dbg * dbg + dp_dba * dba);
        r.segment<3>(ES_V) =
            q_i.conjugate() * (v_j - v_i - dt * gravity) - (dv + dv_dbg * dbg + dv_dba * dba);
