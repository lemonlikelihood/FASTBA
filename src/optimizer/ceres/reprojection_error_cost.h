...
class ReprojectionErrorCost : public Factor::FactorCostFunction,
                              public ceres::SizedCostFunction<2, 4, 3, 3> {
public:
    ...
    bool Evaluate(
        ...
        if (jacobians) {
            ...
            dr_dy << 1.0 / y.z(), 0.0, -y.x() / (y.z() * y.z()), 0.0, 1.0 / y.z(),
                -y.y() / (y.z() * y.z());
            if (jacobians[0]) {
...
public:
    ...
    bool Evaluate(
        const double *const *parameters, double *residuals, double **jacobians) const override {
        std::array<const double *, 3> params = {parameters[0], parameters[1], x.data()};
        if (jacobians) {
            std::array<double *, 3> jacobs = {jacobians[0], jacobians[1], nullptr};
