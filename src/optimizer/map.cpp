...
void Map::print_map() {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        std::cout << "track_id: " << track_cur->track_id
                  << " observation_size:" << track_curr_observation_vec.size() << std::endl;
        for (int j = 0; j < track_curr_observation_vec.size(); j++) {
            std::cout << observation_map[track_curr_observation_vec[j]]->obs_id << " "
                      ...
                      << observation_map[track_curr_observation_vec[j]]->feature->corner.y()
                      << std::endl;
        }
    }
    ...
    for (int i = 0; i < frame_to_other_frame[0][1].size(); ++i) {
        std::cout << observation_map[frame_to_other_frame[0][1][i].first]->track_id << " "
                  ...
                  << " "
                  << observation_map[frame_to_other_frame[0][1][i].first]->feature->corner.y()
                  << std::endl;
        std::cout << observation_map[frame_to_other_frame[0][1][i].second]->track_id << " "
                  ...
                  << " "
                  << observation_map[frame_to_other_frame[0][1][i].second]->feature->corner.y()
                  << std::endl;
        std::cout << std::endl;
    }
    std::cout << "Frame num: " << frame_num << std::endl;
...
double Map::calculate_reprojection_error(bool bal_flag) {
    int count = 0;
    double mse = 0;
    for (auto &obs : observation_map) {
        ...
        // std::cout << "obs_id: " << obs_id << std::endl;
        ...
        // std::cout << "qcw: " << qcw.coeffs() << std::endl;
        // std::cout << "pcw: " << pcw.transpose() << std::endl;
        ...
        Eigen::Vector2d Pn(0, 0);
        if (bal_flag) {
            Eigen::Matrix3d Rcw = qcw.toRotationMatrix();
            Eigen::Vector3d Pc = Rcw * pw + pcw;
            Pn = -Pc.head<2>() / Pc.z();
            double r_p = 1.0 + frame_map[frame_id]->k1 * Pn.squaredNorm()
                         + frame_map[frame_id]->k2 * Pn.squaredNorm() * Pn.squaredNorm();
            Pn = frame_map[frame_id]->f * r_p * Pn;
        } else {
            Eigen::Matrix3d Rcw = qcw.toRotationMatrix();
            Eigen::Vector3d Pc = Rcw * pw + pcw;
            Pn = Pc.head<2>() / Pc.z();
        ...
        mse += res.norm();
        // if (isnan(mse)) {
        //     std::cout << "count: " << count << " mse: " << mse << std::endl;
        //     return 0;
        // }
        count++;
    }
