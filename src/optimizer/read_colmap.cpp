...
void ColmapReader::read_cameras(const std::string &cameras_file)
{
    ...
    while (valid)
    {
        if (line_num > 2)
        {
            ...
            ss >> frame_id >> camera_model >> width >> height >>
...
void ColmapReader::read_images(const std::string &images_file)
{
    ...
    while (valid)
    {
        if (line_num > 3)
        {
            int frame_id;
            ss >> frame_id;
            ss >> map->frame_map[frame_id - 1]->qcw.w() >> map->frame_map[frame_id - 1]->qcw.x() >> map->frame_map[frame_id - 1]->qcw.y() >> map->frame_map[frame_id - 1]->qcw.z();

            ss >> map->frame_map[frame_id - 1]->pcw.x() >> map->frame_map[frame_id - 1]->pcw.y() >> map->frame_map[frame_id - 1]->pcw.z();
...
void ColmapReader::read_points_3D(const std::string &points3D_file)
{
    ...
    while (valid)
    {
        if (line_num > 2)
        {
            ...
            ss >> map->track_map[track_id]->point_3D.x() >> map->track_map[track_id]->point_3D.y() >> map->track_map[track_id]->point_3D.z();
            ss >> map->track_map[track_id]->color[0] >> map->track_map[track_id]->color[1] >> map->track_map[track_id]->color[2];
...
void ColmapReader::associate_OBS()
{
    ...
    for (auto &it : map->frame_map)
    {
        ...
        for (int i = 0; i < frame->keypoints.size(); ++i)
        {
            ...
            map->observation_map[obs_id] = std::make_unique<Observation>(obs_id, frame->frame_id, track_id,
                                                                         frame->keypoints_normalized[i].x(), frame->keypoints_normalized[i].y());
            obs_id++;
        }
    }
