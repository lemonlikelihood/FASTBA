...
// 利用opencv读取image
OpenCvImage::OpenCvImage(const double &t, const std::string &filename) : Image(t, filename) {}

// 传入已有的关键点，返回新的和原有的，图像坐标系
void OpenCvImage::detect_keypoints(
    std::vector<Vector2d> &keypoints, size_t max_points, double keypoint_distance) const {
    if (max_points == 0) {
        max_points = 100;
    }
...
void OpenCvImage::track_keypoints(
    const Image *next_image, const std::vector<Vector2d> &curr_keypoints,
    std::vector<Vector2d> &next_keypoints, std::vector<char> &result_status) const {
    ...
    if (next_cvimage && curr_cvpoints.size() > 0) { // 光流法跟踪
        ...
        calcOpticalFlowPyrLK(
            ...
            cverr); //Size(21, 21), 3, TermCriteria(TermCriteria::COUNT + TermCriteria::EPS, 30, 0.01), OPTFLOW_USE_INITIAL_FLOW);
    ...
    if (l.size() >= 8) { // 如果跟踪到的关键点个数大于8，做一个ransac
        Mat mask;
        findFundamentalMat(
            p, q, cv::FM_RANSAC, 1.0, 0.99,
            mask); // 由于传入的是图像平面坐标，返回的是基本矩阵，如果传入归一化平面坐标，返回的是本质矩阵
...
// 直线检测，返回直线两端点坐标
// void OpenCvImage::detect_segments(
//     std::vector<std::tuple<Vector2d, Vector2d>> &segments, size_t) const {
