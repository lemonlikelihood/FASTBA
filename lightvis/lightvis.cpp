...
struct label_widget_t : public widget_base_t {
    label_widget_t(panel_t *panel, const std::string &label) : label(label), widget_base_t(panel) {}
...
struct progress_widget_t : public widget_base_t {
    progress_widget_t(panel_t *panel, const double &value) : value(value), widget_base_t(panel) {}
...
public:
    ...
    void load() {
        ...
        static const gl::GLchar *grid_fshader = R"(
            ...
            void main(){
                ...
                vec4 c = vec4(Frag_Color.rgb, Frag_Color.a * max(min(min(r.x, r.y), r.z), 0.9));
                Out_Color = c;
            }
        )";
    ...
    void draw_grid() {
        ...
        grid_shader->set_uniform(
            "Color", Eigen::Vector4f {1.0, 1.0, 1.0, float(pow(level - floor(level), 0.9) * 0.25)});
        grid_shader->set_attribute("Position", grid_lines);
        grid_shader->draw(gl::GL_LINES, 0, grid_lines.size());
    ...
    void create_window() {
        ...
        gl::glVertexAttribPointer(
            (gl::GLuint)context.attribute_position, 2, gl::GL_FLOAT, gl::GL_FALSE, sizeof(vertex_t),
    ...
    static void window_refresh_callback(GLFWwindow *win) {
        ...
        vis->gui(
            &vis->detail->context.nuklear, vis->detail->viewport.window_size.x(),
